...
import { fetchLogsCommand } from './fetchLogs';  
import { marked } from "marked"; // <-- Added for formatting
...
export async function activate(context: vscode.ExtensionContext) {
 ...
 // ------------------ INIT PROJECT LOGGER ------------------
ProjectLogger.init();
ProjectLogger.log("🔵 Extension activated and logger started");

// ------------------ GENERATE LOGS BUTTON COMMAND ------------------
const logCmd = vscode.commands.registerCommand(
  "aicodereview.generateLogs",
  async () => {
    const file = ProjectLogger.flushToDisk();
    if (file) {
    ...
    async () => {
      try {
        await vscode.commands.executeCommand(
          "workbench.view.extension.aiCodeReview"
        );
...
ProjectLogger.log("Git logs watching--");
    output.appendLine(`🔍 Watching Git log at: ${gitLogPath}`);

    const watcher = fs.watch(gitLogPath, async (eventType: string) => {
      if (eventType === "change") {
    ...
    cp.exec(
      ...
      (error, stdout, stderr) => {
        if (error) {
          reject(
            new Error(
              `❌ Git diff failed in ${workspacePath}: ${
...
async function getBackendReview(diff: string): Promise<string> {
...
// ----------------------------------------------------------------------
async function runAIReview(output: vscode.OutputChannel): Promise<string> {
  output.appendLine("🤖 Starting AI Code Review...");
ProjectLogger.log("Starting AI code Reviewer!");
...
ProjectLogger.log("🛑 Extension deactivated");
...
function captureGitLogsOnStart() {
  const workspaceFolders = vscode.workspace.workspaceFolders;
  if (!workspaceFolders || workspaceFolders.length === 0) {
    ProjectLogger.log("⚠️ No workspace folder — Git logs not captured.");
    return;
  }

  const workspacePath = workspaceFolders[0].uri.fsPath;

  ProjectLogger.log("📥 Capturing Git logs on startup...");

  const gitCommand =
    'git --no-pager log --pretty=format:"commit %H%nAuthor: %an <%ae>%nDate:   %ad%n%n    %s%n" --date=local -n 20';

  cp.exec(gitCommand, { cwd: workspacePath }, (err, stdout, stderr) => {
    if (err) {
      ProjectLogger.log(`❌ Git log error: ${err.message}`);
      return;
    }

    if (stderr && stderr.trim() !== "") {
      ProjectLogger.log(`⚠️ Git stderr: ${stderr}`);
    }

    if (stdout && stdout.trim() !== "") {
      ProjectLogger.log("📄 Git Logs (Clean Format):");
      ProjectLogger.log(stdout);
    } else {
      ProjectLogger.log("ℹ️ No Git logs found.");
    }
  });
}
