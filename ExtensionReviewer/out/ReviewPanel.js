...
class ReviewPanel {
    ...
    async resolveWebviewView(webviewView) {
        ...
        if (!this._token) {
            ...
            webviewView.webview.html = this._getHtml(this._content, this._hasGeneratedTests);
        }
        webviewView.webview.onDidReceiveMessage(async (message) => {
            switch (message.command) {
                case "login":
                    ...
                    break;
                case "reanalyze":
                    this.update('<div><span class="spinner"></span> Re-analyzing the code...</div>');
                ...
                case "generateTest":
                    try {
                        ...
                        this.update('<div><span class="spinner"></span> Generating test cases...</div>');
                        vscode.window.showInformationMessage("Generating The Test Case...!");
                        const diff = await (0, extension_1.getGitDiff)();
                        if (!diff || diff.startsWith("⚠️")) {
                            this.update("⚠️ No new commit diff found.");
                            return;
                        }
                        const response = await fetch("http://localhost:5142/api/test/generate", {
                            method: "POST",
                            headers: {
                ...
                case "validateTest":
                    try {
                        ...
                        this.update('<div><span class="spinner"></span> Validating test cases...</div>');
                        vscode.window.showInformationMessage("Validating The Test Case...!");
                ...
                case "saveTest":
                    try {
                        ...
                        vscode.window.showInformationMessage(`💾 Test saved to tests/${fileName}`);
                    }
                    catch (err) {
                        vscode.window.showErrorMessage(`❌ Failed to save test: ${err.message}`);
                    }
                    break;
                case "logout":
    ...
    _refreshSidebar() {
        ...
        if (!this._token) {
            ...
            this.view.webview.html = this._getHtml(this._content, this._hasGeneratedTests);
        }
    }
    _getLoginHtml() {
        return `
    ...
    _getHtml(content = this._content, showValidate = false) {
        return `
    <html>
    <head>
      <style>
...
ReviewPanel.viewType = "aiCodeReview.panel";
